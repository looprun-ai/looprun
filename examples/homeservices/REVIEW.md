...
- **Inputs used:** the purpose sentence (Q0) + simulated-user defaults — NO external material,
  no gold/certified specs, contracts or evals were read (anti-contamination law).
- **Pipeline executed:** A → G1 (tool genesis, isolated; artifacts flowed to the engineers —
  ...
  E1 (decompose) → E2 (draft ×2) ∥ E3 (contract) → N (5 reviewers + verifier, 2 rounds).
...
**Contract summary:** locale English; voice = BrightNest operations assistant (professional,
...
| N2-1 | Bucket-A auditor | all behavior lines + precondition proses state their CONDITION; contract invariants are protocol/never rules, no state snapshots | — | NO FINDINGS |
...
| N4-2 | coverage critic | confidentiality (contract invariant 5) has no eval case | PLAUSIBLE | ACCEPTED residual (case budget at 22 cap; logged in EVALS.md) |
