 ...
 *     exemption. A bare question mark ("…sent! Anything else?") must NOT bypass a declarative claim.
 */
 ...
 * A truthful "that invoice is already paid — it cannot be voided" or "the filing is not yet prepared"
 * must PASS the claim gate. Shared across the domain's specs (one source, no per-agent restating).
 */
 ...
 *
 * ATTEMPT-CONTEXT failure verbs ONLY (the default-template rule, measured 2026-07-16): this guard fires
 * on all-calls-succeeded turns, which is exactly the shape of an HONEST policy refusal after clean reads
 * ("cannot void a paid invoice", "could not release — open claim"). Generic inability/refusal words
 * (`cannot`, `unable`, `could not process|complete`) would match those honest refusals and exhaust the
 * redrive into the fallback stub — so they are deliberately EXCLUDED. Only "the attempt broke" phrasing
 * qualifies.
 */
