...
| R6.5 | No rule interprets the USER'S text. Guards read state, calls, results, and the model's own output. The engine's only touch on user text is searching for literals it minted itself (consent codes). A rule that turns on judging text is a judged question on the session's own model — never a pattern over conversation text. | the intent-magnet firewall (D3) + prompt-injection defense; this is what keeps the guard layer model- and language-independent |
| R6.6 | The engine ships NO linguistic pattern and accepts none (no regex over reply/request as a validation decision; no shared lexicon module). Language-specific judgment belongs to judged questions or to the bundle's own declarations. | multilingual fragility measured; a pattern list is always a partial language |
...
[ ] any validation decision runs a pattern over model or user text
