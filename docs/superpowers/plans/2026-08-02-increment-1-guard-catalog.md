...
- [ ] **Step 1: Failing tests** — three fixtures:
...
- [ ] **Step 1: Failing test** — install a `requiresBefore` guard via the loader against a fixture world holding figures (e.g. `limits: {seats: 2}`), trigger the deny, and assert: `expect(deny).not.toMatch(/\d/)` and `expect(deny).toMatch(/getPlanUsage/)`. (The policy IS the test: names the read, never the figures.)
- [ ] **Step 2: FAIL** (loader currently passes reasons through). **Step 3:** route all catalog deny text through `renderDeny`; configs cannot override it (no `reason` field in the schema — remove it if Step 3 of Task 2 added one). **Step 4:** green + typecheck. **Step 5: Commit** — `feat(eval): deny policy — catalog denies name the read, never the figures`.
