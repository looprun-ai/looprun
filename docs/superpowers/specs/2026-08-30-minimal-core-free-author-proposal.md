...
| hand-written cards.ts is the format the emitter was CREATED to take away from the author — the declared path exists because agents erred it | OPEN — not weighed against the historical failure record; before step 6 ships, a micro-test puts a blind agent on hand-editing cards.ts under the full net (tsc + CardCheck + the 27-lint gate) and classifies what still gets through vs the declaration era's error classes |
