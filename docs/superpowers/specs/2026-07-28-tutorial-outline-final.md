...
> **CLOSED.** This outline is the standing record of the public-API contract: the six `src/index.ts`
> barrels and their surface-lock tests cite it by path. Every §6 decision is settled — the two that
> were the user's are recorded, with their reasons, in `2026-07-28-migration-notes.md`.
...
primitives (`createActionHistory`, `evaluatePreTool`, `finalizeReply`, …) for enforcing a spec inside a
