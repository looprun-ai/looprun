...
The naked variant's only irreplaceable job was raw non-vacuity ("does this rule change
anything at all"), and the prose-only baseline answers that question almost as well (a
case both variants pass does not bite, whatever the baseline). Decision: **prose-only
REPLACES naked** (option B′). No third variant; naked dies with no alias.
...
| `stateBlock` on the tail, terminal policy, maxSteps, sampling | ON | loop mechanics, identical in both variants |
...
3. **Byte-identity gate**: a test asserts the ungov variant's assembled system prompt equals
   the governed variant's, byte for byte. This invariant IS the design; if it cannot hold
   (e.g. the override seam renders differently), the implementation is wrong, not the test.

## CLI / artifacts

- Flag stays `--ungoverned`; variant label stays `ungoverned` in dumps (no compat concern
...
a. Byte-identity of the assembled system prompt between variants.
...
- No third variant; no `--variant naked`.
