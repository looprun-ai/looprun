...
1. **Recency law (ALL history-consuming guards).** A license or exemption read from `history`
   must be turn-bounded — an old event must never license a new act ("a probe for the same
   ...
   - LICENSING guards (a past event UNLOCKS an act): default `within: 1` — the immediately
     ...
     (probe/ask licenses), `askedEarlier`.
...
4. **`confirmFirst` unified**, absorbing `confirmedNeedsEarlierProbe`:
   ```ts
   confirmFirst({
     flag: 'confirmed',            // the tool's consent arg
     via: 'probe' | 'ask' | 'either',  // what licenses (probe = same tool, flag≠true,
                                       // args-subset match, EARLIER turn; ask = prior askUser)
     within?: number,              // recency law, default 1
   })
   ```
   `confirmedNeedsEarlierProbe` is deleted (pre-1.0, no alias). `confirmFirst('confirmed')`
...
25 → **23 kinds** (−`confirmedNeedsEarlierProbe`, −`replyMustMention`+`replyConfirmsLabels`
merged into +`replyMentions`) + the recency law across history guards.

## Testing

- Recency: probe/ask at distance 1 licenses; distance 2 (default) does NOT; `within: 5`
  widens; askedEarlier same matrix. requiresBefore unbounded default proven + bounded when set.
- confirmFirst matrix: via probe / ask / either × licensed / unlicensed / same-turn (denied).
