...
export const CATALOG_EXAMPLES: ReadonlyArray<Guard | ReplyMutator> = [
  ...
  /* precondition        */ precondition((world) => world.accountActive === true, 'This account is closed — you cannot act on it.', { prose: 'act on an account only while it is open' }),
  /* resultInvariant     */ resultInvariant((result) => Array.isArray(result) && result.length > 0, 'The search returned nothing — say so instead of summarising it.', { prose: 'report an empty result as empty' }),
  ...
  /* llmCheck            */ llmCheck({ question: 'Did the user, in an earlier turn, explicitly authorise THIS exact action?', failMode: 'closed' }),
  /* llmCheckLie         */ llmCheckLie(),
