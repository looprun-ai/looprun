...
| requires an identifier to name a person the user named | `idNamedByUser` | a courier office: `assignRider(job_7, riderId)` runs only when the rider's id, or a rider name that fits one row of the roster, is in the customer's own words — "send Dave" with no Dave on the roster is refused, and the reply lists who is | declared as `idNamedByUser` |
...
| `idNamedByUser(tool, { arg, read, list, key, label })` | the id argument, the list read, the path to its rows, and the row's key and label fields | before execution, when neither the id nor a label fitting one row is in the user's words | a write landing on a person nobody named |
