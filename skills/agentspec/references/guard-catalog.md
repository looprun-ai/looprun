...
| `confirmFirst(opts?: string \| { argFlag?, mechanism? })` | run | **base** | destructive-confirm gate keyed by MECHANISM. `'arg'` (default; a bare string sets `argFlag`): `confirmed:true` needs a prior-turn `confirmed:false` PROBE. `'prior-ask'` (flag-less tools): legal only after an `askUser` in an EARLIER turn (compose with `noActAfterAskSameTurn` for the same-turn edge). Auto-installed per `cfg.confirmMechanism`. **PROSE half MUST carry the FOLLOW-THROUGH** (measured 2026-07-17 — the largest deterministic two-step fail cluster): state explicitly "after the go-ahead in a LATER turn, call the tool AGAIN with `confirmed:true`" — the check gates the premature confirm, but only the prose prevents the opposite defect (re-probing `confirmed:false` forever after the user agreed; `pendingConfirmMustAsk` then forces re-asking → a politeness loop). See decompose-and-draft.md "Confirm FOLLOW-THROUGH line" |
