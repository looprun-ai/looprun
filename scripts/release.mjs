 ...
 *   3. gates       pnpm -r build + typecheck + test, then `pnpm gates` — the repo's five gates:
 *                  no external model, no secret, no retired plain name, the guard priority
 *                  order, no bench drift
...
const gates = () => {
  step('gates: clean + build + typecheck + test + the five repo gates');
  ...
  // The repo's own gates, the one list `pnpm gates` keeps: no external model, no secret, no
  // retired plain name, the guard priority order, no bench drift.
  run('pnpm gates');
};
