...
import { AgentSpecBase, custom, mustAccountFor, requiresBefore } from '@looprun-ai/core';
...
function cleanSpec() {
  ...
  spec.addReplyCheck(mustAccountFor({ records: ['REF-1'], outcome: 'any' }, 'name the reference'), { id: 'agent:labels' });
  return spec;
}
...
describe('lintSpecExecution — unsatisfiable reply requirements', () => {
  ...
  it('a throwing check() is its own finding, not a crash', async () => {
    ...
    spec.addReplyCheck(mustAccountFor({ records: ['REF-1'], outcome: 'any' }, 'name it'), { id: 'agent:labels' });
    spec.addReplyCheck(
