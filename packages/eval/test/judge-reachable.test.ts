/**
 * A subject that binds a question reaches its first turn through the eval package's OWN dependency
 ...
 * `llmCheck` question aborts before its first reply.
 */
...
function questionBoundSpec(): AgentSpecBase {
  const spec = new AgentSpecBase({ id: 'reachable', mode: 'M', persona: 'You are the agent.', tools: [], contract: CONTRACT });
  spec.addGuard('onReply', 'any', llmCheck({ question: 'does the reply overstate the result?' }), { id: 'agent:question' });
  return spec;
}

describe('a bound question through the eval package own runner path', () => {
  it('does not abort at conversation start with NO judge in deps', async () => {
    ...
    const res = await runSpecConversation(questionBoundSpec(), [{ userText: 'hello' }], {
