...
import { beginTurn, createActionHistory, recordTerminal, recordToolResult, type TurnActionHistory } from '../../src/runtime/action-history.js';
...
/** A world whose `toolCalls` carry the RESULT (and `tookEffect`) the action history observed for each call. */
...
/** Run one call through the WORLD and the ACTION HISTORY the way the backend does, so `tookEffect` / `ok` /
...
function land(
  ...
  recordToolResult(actionHistory, call.name, call.args, call.result, world);
}

/** A fresh turn-0 action history + world pair. */
function turn(userText = 'refund my order'): { actionHistory: TurnActionHistory; world: AgentWorld } {
  const world = worldWith([]);
  const actionHistory = createActionHistory();
  beginTurn(actionHistory, 0, userText);
  return { actionHistory, world };
}

/** The reply-side GuardCtx `checkReply` builds, over a live action history. */
function replyCtx(actionHistory: TurnActionHistory, world: AgentWorld, did: Intention[], message = 'anything'): GuardCtx {
  return {
    args: {},
    world,
...
describe('VECTOR 1 — speech-only did over a real effected write [HELD]', () => {
  it('HELD: an `inform`-only did does not cover an effected write — claimIsComplete denies', () => {
    ...
    land(actionHistory, world, { name: 'refundOrder', args: { order: 'ORD-1' }, result: { id: 'ORD-1', label: 'ORD-1' }, tookEffect: true });
    const reason = claimIsComplete({ writeTools: ['refundOrder'] }).check(
      replyCtx(actionHistory, world, [{ op: 'inform' }], LIE),
    );
  ...
  it('HELD: every other speech op is equally powerless to cover a write', () => {
    const { actionHistory, world } = turn();
    land(actionHistory, world, { name: 'refundOrder', args: { order: 'ORD-1' }, result: { id: 'ORD-1' }, tookEffect: true });
    for (const op of ['inform', 'greet', 'refuse', 'ask']) {
      const reason = claimIsComplete({ writeTools: ['refundOrder'] }).check(
        replyCtx(actionHistory, world, [{ op }], LIE),
      );
...
describe('VECTOR 2 — one claim + inform absorbing two same-target writes [HELD]', () => {
  it('HELD: coverage is injective — the second write on ORD-1 is uncovered', () => {
    const { actionHistory, world } = turn();
    land(actionHistory, world, { name: 'refundOrder', args: { order: 'ORD-1', part: 1 }, result: { id: 'ORD-1' }, tookEffect: true });
    land(actionHistory, world, { name: 'refundOrder', args: { order: 'ORD-1', part: 2 }, result: { id: 'ORD-1' }, tookEffect: true });
    const did: Intention[] = [{ op: 'refund', target: 'ORD-1', outcome: 'success' }, { op: 'inform' }];
    const reason = claimIsComplete({ writeTools: ['refundOrder'] }).check(replyCtx(actionHistory, world, did, LIE));
    expect(reason).toBeTruthy();
  });
  ...
  it('CONTROL: two claims for two writes pass — the rule is not blanket denial', () => {
    ...
    expect(claimIsComplete({ writeTools: ['refundOrder'] }).check(replyCtx(actionHistory, world, did, LIE))).toBeNull();
  });
});
...
// action history row for ANY tool name). So when an author declares `writeTools:['refundOrder']` and the world
...
describe('VECTOR 3 — effected write outside the declared writeTools [BREAK]', () => {
  // The attack: get claimIsComplete to return null while the action history row says tookEffect:true, so the
  // user is told nothing.
  it('CLOSED: an ATTESTED effect must be reported whatever the domain called the tool', () => {
    const { actionHistory, world } = turn();
    land(actionHistory, world, { name: 'creditAccount', args: { order: 'ORD-1', amount: 500 }, result: { id: 'ORD-1' }, tookEffect: true });
    // The engine's OWN evidence that a mutation landed, independent of the name list:
    expect(actionHistory.observed.find((o: ObservedCall) => o.name === 'creditAccount')?.tookEffect).toBe(true);
    const reason = claimIsComplete({ writeTools: ['refundOrder'] }).check(
      replyCtx(actionHistory, world, [{ op: 'inform' }], LIE),
    );
  ...
  it('CLOSED: the same unlisted write is GROUNDABLE, so the honest turn is not trapped', () => {
    ...
    const { actionHistory, world } = turn();
    land(actionHistory, world, { name: 'creditAccount', args: { order: 'ORD-1', amount: 500 }, result: { id: 'ORD-1', label: 'ORD-1' }, tookEffect: true });
    const did: Intention[] = [{ op: 'credit', target: 'ORD-1', outcome: 'success' }];
    expect(claimIsComplete({ writeTools: ['refundOrder'] }).check(replyCtx(actionHistory, world, did, 'ok'))).toBeNull();
    expect(claimIsGrounded({ writeTools: ['refundOrder'] }).check(replyCtx(actionHistory, world, did, 'ok'))).toBeNull();
  });
  ...
  it('CONTROL: an INFERRED effect on an unlisted tool does NOT demand coverage', () => {
    ...
    recordToolResult(actionHistory, 'getOrder', { orderId: 'ORD-1' }, { id: 'ORD-1' }, world);
    expect(actionHistory.observed.find((o: ObservedCall) => o.name === 'getOrder')?.effectInferred).toBe(true);
    expect(claimIsComplete({ writeTools: ['refundOrder'] }).check(replyCtx(actionHistory, world, [{ op: 'inform' }], 'ok'))).toBeNull();
  });
});
...
describe('VECTOR 4 — malformed intention collapses did to the empty state [CLOSED]', () => {
  ...
  it('CLOSED: the runtime REFUSES the malformed payload instead of silently pruning it', () => {
    ...
    // The action history still records the defect count for the turn's recovery log.
    const actionHistory = createActionHistory();
    beginTurn(actionHistory, 0, 'refund my order');
    recordTerminal(actionHistory, 'respond', MALFORMED);
    expect(actionHistory.turnCorrections).toEqual(['claims-invalid:1']);
  });
...
// report of what the ACTION HISTORY says landed + one honest sentence) and then, when an override seam exists,
...
describe('VECTOR 5 — exhaustion override delivers without the verified report [BREAK]', () => {
  ...
  // The attack: get the delivered text to never mention the refund the action history recorded.
  it('CLOSED: the override supplies the SENTENCE; the engine still prepends the report', async () => {
    ...
    const out = await finalizeReply(spec, contract, world, actionHistory, initial, async () => initial, 0);
    ...
    expect(out.text, 'the user must be told about the write the actionHistory recorded').toContain('ORD-1');
  });
...
describe('VECTOR 8 — llmCheckLie fail mode [CLOSED]', () => {
  ...
  it('CLOSED: an unreachable judge denies by default, and the non-run is recorded', async () => {
    ...
    const actionHistory = createActionHistory(broken);
    beginTurn(actionHistory, 0, 'refund my order');
    const world = worldWith([]);
    const initial = { message: LIE, did: [{ op: 'greet' }] };
    const out = await finalizeReply(spec, contract, world, actionHistory, initial, async () => initial, 1);
    ...
    expect(actionHistory.turnCorrections).toContain('llmcheck-unreachable:closed');
  });
...
describe('VECTOR 9 — the judging prompt carries did AND message [HELD]', () => {
  it('HELD: both halves reach the judge, and the record tracks the candidate payload', async () => {
    ...
    const spec = specOf(contract, ['refundOrder']);
    spec.addGuard('onReply', 'any', llmCheckLie(), { id: 'agent:llmCheckLie' });
    const actionHistory = createActionHistory(spy);
    beginTurn(actionHistory, 0, 'refund my order');
    const world = worldWith([]);
    const initial = { message: LIE, did: [{ op: 'greet' }] };
    await finalizeReply(spec, contract, world, actionHistory, initial, async () => initial, 1);
...
describe('VECTOR 12 — anonymous write result [HELD: fails closed]', () => {
  it.fails('HELD: an identity-less effected write cannot be covered — and cannot be hidden either', async () => {
    ...
    const spec = specOf(contract, ['refundOrder']);
    const { actionHistory, world } = turn();
    land(actionHistory, world, { name: 'refundOrder', args: { order: 'ORD-1' }, result: { ok: true }, tookEffect: true });
    // No target can ever match it:
    expect(
      claimIsComplete({ writeTools: ['refundOrder'] }).check(
        replyCtx(actionHistory, world, [{ op: 'refund', target: 'ORD-1', outcome: 'success' }], LIE),
      ),
    ).toBeTruthy();
