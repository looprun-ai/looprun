 ...
 *
 * The user-facing OPERATION REPORT comes from the action history-verified `did` rendered BY THE ENGINE, never from
 * the agent's free prose — so no fabricated claim reaches the user THROUGH THE REPORT, and a
 * redrive/exhaustion never announces an effect the action history does not show. The agent's own `message` still
...
import { renderOperationReport, deriveClaimsFromActionHistory, hasAskIntent } from '../src/internal.js';
import type { Intention } from '../src/runtime/claims.js';
import { createActionHistory, recordToolResult, recordTerminalCall } from '../src/runtime/action-history.js';
...
/** Seed a WRITE that TOOK EFFECT, aligning the world action history (what resultOf reads) with the observed entry
 *  (recordToolResult copies tookEffect from the world match). `label` is the world's produced noun. */
function effectWrite(actionHistory: ReturnType<typeof createActionHistory>, world: AgentWorld, name: string, args: Record<string, unknown>, label: string): void {
  world.toolCalls.push({ name, args, result: { id: label, label }, tookEffect: true });
  recordToolResult(actionHistory, name, args, { id: label, label }, world);
}

/** Seed a WRITE that RAN OK but changed nothing (a simulate: tookEffect false). */
function simulateWrite(actionHistory: ReturnType<typeof createActionHistory>, world: AgentWorld, name: string, args: Record<string, unknown>): void {
  world.toolCalls.push({ name, args, result: { success: true }, tookEffect: false });
  recordToolResult(actionHistory, name, args, { success: true }, world);
}
...
describe('deriveClaimsFromActionHistory — the engine derives TRUTH from the world actionHistory', () => {
  it('effected write → success with the produced label as target; a read + a simulate contribute nothing', () => {
    ...
    effectWrite(actionHistory, world, 'createBooking', { slot: 1 }, 'BK-1'); // effected
    simulateWrite(actionHistory, world, 'createBooking', { slot: 2 });          // simulate, no effect
    recordToolResult(actionHistory, 'getMember', { id: 7 }, { name: 'Ana' }); // a read
  ...
  it('a READ that emitted a label does NOT lend it to a later effected write', () => {
    ...
    recordToolResult(actionHistory, 'search', { q: 'x' }, { label: 'SEARCH-RESULT' }, world); // a READ with a label
    world.toolCalls.push({ name: 'createBooking', args: { slot: 1 }, result: { ok: true }, tookEffect: true });
    recordToolResult(actionHistory, 'createBooking', { slot: 1 }, { ok: true }, world);       // a WRITE with none
    const derived = deriveClaimsFromActionHistory(actionHistory.observed, 0, ['createBooking']);
  ...
  it('two effected writes each wear THEIR OWN label (no positional cursor to misalign)', () => {
    const actionHistory = createActionHistory();
    const world = fixtureWorld();
    effectWrite(actionHistory, world, 'createBooking', { slot: 1 }, 'BK-1');
    world.toolCalls.push({ name: 'search', args: { q: 'y' }, result: { label: 'SEARCH-RESULT' }, tookEffect: false });
    recordToolResult(actionHistory, 'search', { q: 'y' }, { label: 'SEARCH-RESULT' }, world); // a read in between
    effectWrite(actionHistory, world, 'createBooking', { slot: 2 }, 'BK-2');
    const derived = deriveClaimsFromActionHistory(actionHistory.observed, 0, ['createBooking']);
  ...
  it('an effected write carrying requiresConfirmation derives as SUCCESS, not pending', () => {
    ...
    recordToolResult(actionHistory, 'refund', { order: 'ORD-7' }, { label: 'ORD-7', requiresConfirmation: true }, world);
    const derived = deriveClaimsFromActionHistory(actionHistory.observed, 0, ['refund']);
  ...
  it('a world label colliding with a SPEECH op never becomes the derived `op`', () => {
    ...
    effectWrite(actionHistory, world, 'createTicket', { q: 1 }, 'ask');
    const derived = deriveClaimsFromActionHistory(actionHistory.observed, 0, ['createTicket']);
  ...
  it('every SPEECH op is coerced; an ordinary label is untouched', () => {
    for (const speech of ['inform', 'greet', 'refuse', 'ask']) {
      const actionHistory = createActionHistory();
      const world = fixtureWorld();
      effectWrite(actionHistory, world, 'createTicket', { q: 1 }, speech);
      expect(deriveClaimsFromActionHistory(actionHistory.observed, 0, ['createTicket'])).toEqual([
        { op: 'operation', target: speech, outcome: 'success' },
      ]);
    }
    const actionHistory = createActionHistory();
    const world = fixtureWorld();
    effectWrite(actionHistory, world, 'createTicket', { q: 1 }, 'BK-9');
    expect(deriveClaimsFromActionHistory(actionHistory.observed, 0, ['createTicket'])).toEqual([
...
describe('finalizeReply — composed delivery over the structured payload', () => {
  ...
  it('empty did → NOT deliverable: the declaration floor drives the turn to the engine closure', async () => {
    ...
    const out = await finalizeReply(spec, BOOKING_CONTRACT, fixtureWorld(), createActionHistory(), P('Hello there.'), async () => P(''), 0);
  ...
  it('CONTROL: a speech-only did delivers the message under the empty-case closure, unexhausted', async () => {
    const spec = new AgentSpecBase({ id: 'a', mode: 'M', persona: 'p', tools: [], contract: BOOKING_CONTRACT });
    const out = await finalizeReply(spec, BOOKING_CONTRACT, fixtureWorld(), createActionHistory(), P('Hello there.', [{ op: 'greet' }]), async () => P(''), 0);
  ...
  it('non-empty VERIFIED did → message + the engine-rendered operation report', async () => {
    ...
    const out = await finalizeReply(spec, BOOKING_CONTRACT, world, actionHistory, P('All booked.', did), async () => P(''), 0);
    expect(out.text).toBe('All booked.\n\nBK-1: done\nNothing else was changed on this turn.');
    expect(out.exhausted).toBe(false);
    expect(out.did).toEqual(did);
    // History keeps the VERIFIED set: finalizeReply synced the action history to what it delivered.
  ...
  // must survive into `out.did` AND `action history.did` (history).
  it('an accepted payload KEEPS its ask intention in the verified did (the consent record)', async () => {
    const spec = new AgentSpecBase({ id: 'a', mode: 'M', persona: 'p', tools: ['createBooking'], contract: BOOKING_CONTRACT });
    const actionHistory = createActionHistory();
    const did: Intention[] = [{ op: 'ask' }];
    const out = await finalizeReply(spec, BOOKING_CONTRACT, fixtureWorld(), actionHistory, P('Shall I cancel it?', did), async () => P(''), 0);
...
describe('finalizeReply — the claims-derived exhaustion closure never fabricates and never leaks a tool name', () => {
  it('an effected write the model FAILED to declare → engine derives the truth (produced label, not tool name)', async () => {
    ...
    const out = await finalizeReply(spec, BOOKING_CONTRACT, world, actionHistory, P('Anything else?'), async () => P('Anything else?'), 0);
    expect(out.exhausted).toBe(true);
    ...
    expect(actionHistory.did).toEqual(out.did); // history retains the derived truth
  });
  ...
  it('a simulate-only turn → the "nothing was changed" shape, no fabricated success', async () => {
    ...
    const actionHistory = createActionHistory();
    const world = fixtureWorld();
    simulateWrite(actionHistory, world, 'createBooking', { slot: 9 }); // ran, changed nothing
    const out = await finalizeReply(spec, BOOKING_CONTRACT, world, actionHistory, P('Working on it.'), async () => P('Working on it.'), 0);
    expect(out.exhausted).toBe(true);
...
describe('finalizeReply — redrive returns a whole payload, re-checked against the actionHistory', () => {
...
describe('finalizeReply — salvage re-validates the FULL payload, so a fabricated did is never salvaged', () => {
  it('rejects a salvage candidate whose did fails claimIsGrounded → falls through to the honest closure', async () => {
    ...
    expect(out.text).toBe('No operation was carried out on this turn.\n\nI could not complete this safely — nothing was changed. Could you rephrase or add detail?');
    expect(actionHistory.turnCorrections.some((c) => c.startsWith('salvage-miss:checks:claimIsGrounded'))).toBe(true);
  });
});
