 ...
 *   1. the meta-bearing kinds (requiresBefore → before, mustAccountFor → requiredStrings, its targets)
...
import { AgentSpecBase, mustAccountFor, requiresBefore } from '../../src/index.js';

describe('guard meta introspection', () => {
  it('requiresBefore attaches meta.before; mustAccountFor attaches meta.requiredStrings (its targets)', () => {
    expect(requiresBefore(['a', 'b']).meta).toEqual({ before: ['a', 'b'] });
    expect(mustAccountFor({ records: ['x', 'y'], outcome: 'any' }, 'r').meta).toEqual({ requiredStrings: ['x', 'y'] });
    expect(mustAccountFor({ records: ['L-1'], outcome: 'success' }, 'r').meta).toEqual({ requiredStrings: ['L-1'] });
  });
  ...
  it('meta survives the attributeGuard wrapper installed by AgentSpecBase.addGuard', () => {
    ...
    spec.addReplyCheck(mustAccountFor({ records: ['L-9'], outcome: 'any' }, 'name it'), { id: 'agent:labels' });
