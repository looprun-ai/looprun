...
/** The desk closing its own turn, paying every fact the close instruction numbered:
 *  every id, figure and code the gate owes is carried, and the report accounts for the
 *  held cancellation the turn recorded. */
const HELD_ROW = [{ tool: 'cancelBooking', target: 'bk_9', word: 'held' }];

const paysClose = (input: StepInput, prefix = 'Closing.'): ModelStep =>
  finishStep(`${prefix} ${closeWords(input)}`, HELD_ROW, closeIds(input));
...
test('an owed literal the message drops is refused before it is delivered', async () => {
...
test('the redrive runs the same model on the same prefix — the correction is appended', async () => {
  ...
  const loop = port.seen.filter(s => !isClose(s));
...
test('the owed facts reach the desk numbered, before it writes', async () => {
  ...
  const closing = port.seen.filter(s => !isClose(s)).slice(-1)[0];
...
test('a figure the message states and no record carries is refused, then rewritten', async () => {
  const port = closingPort([
    callStep('cancelBooking', { id: 'bk_9' }),
    finishStep('Cancelled room 12. Seven nights would come to 364.',
      [{ tool: 'cancelBooking', target: 'bk_9', word: 'done' }], ['F1']),
  ...
  const walked = redrivesFor(r2, 'figureIsGrounded');
...
test('a figure ungrounded on every attempt floors — the operator never reads it', async () => {
  const dirty = finishStep('Cancelled room 12. Seven nights would come to 364.',
    [{ tool: 'cancelBooking', target: 'bk_9', word: 'done' }], ['F1']);
  const port = closingPort([
    callStep('cancelBooking', { id: 'bk_9' }), dirty, dirty, dirty
  ], input => paysClose(input, 'Closing. Seven nights would come to 364.'));
  const { engine } = caseRig({ model: port, contract: HELD_CANCEL });

  const r1 = await engine.chat('s1', 'cancel booking bk_9');
  const r2 = await engine.chat('s1', r1.questions.issued[0].code);

  expect(r2.delivery.by).toBe('floor');
  expect(r2.text).not.toContain('364');
  expect(r2.text).toContain('Cancelled room 12.');
});

test('a message whose figures the records carry seals as the desk wrote it', async () => {
  const port = closingPort([
    callStep('cancelBooking', { id: 'bk_9' }),
    finishStep('Room 12 stands cancelled.',
      [{ tool: 'cancelBooking', target: 'bk_9', word: 'done' }], ['F1'])
  ], input => paysClose(input));
  const { engine } = caseRig({ model: port, contract: HELD_CANCEL });
  ...
  expect(redrivesFor(r2, 'figureIsGrounded')).toEqual([]);
  expect(r2.delivery.by).toBe('prose');
  expect(r2.delivery.retried).toBe(false);
  expect(r2.text).toBe('Room 12 stands cancelled.');
});
