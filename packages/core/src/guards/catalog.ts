...
export const GUARD_CATALOG: readonly GuardCatalogEntry[] = [
  ...
  },
  {
    ...
    whenToUse:
      'Auto-installed alongside `confirmFirst`. It is the blast-radius cap, not a consent gate: it stops chained destructive calls in one turn even when each one is individually confirmed. A call that carries `simulate: true` (or whose result came back asking for confirmation, having changed nothing) reads as a simulation and does not count — so a legitimate multi-simulation is not vetoed, while a BARE same-step sibling counts as the effect it will be and the cap holds from the first one. It shares the spec\'s `destructiveWhen` predicates, so the cap counts destructive acts: three protective calls in one turn are three legitimate calls, and the second destructive one is still stopped.',
    example: `destructiveThrottle(['cancelBooking', 'purgeAccount'])`,
  },

  // ── honesty ────────────────────────────────────────────────────────────────
  // THE HONESTY KINDS ARE STRUCTURAL, NOT TEXTUAL: each reads the agent's declaration (`ctx.did`)
  // against the world action history, never the reply prose, so it carries no pattern and no polarity that a
