...
1. **Tool wiring with a pre-call veto** — before a tool executes, run `evaluatePreTool(spec,
   action history, world, tool, args)`; on deny, short-circuit the call and hand the model the correction
...
3. **Reply finalization** — `finalizeReply(spec, contract, world, action history, text, redrive, redrives)`
