/**
 * @looprun/vercel — RESERVED backend slot (not implemented in v0).
 *
 * The seam contract a backend implements (everything else lives framework-free in @looprun/core):
...
export function createLoopRunAgent(_config: VercelBackendConfig): never {
  throw new Error('@looprun/vercel: not implemented — the v0 backend is @looprun/mastra.');
}
