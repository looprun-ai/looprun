...
- Release (minor).
- db93b7d: The live house: `RoutedAgent.fromLiveSubject` seats a house of desks over the host's own tools —
  every desk on the same live map and seal, the marked default or the engine's front of house
  taking what no desk matched. `desk(name)` is a desk of the house for a turn the host pins;
  `settle()` on both agent classes awaits construction once, so a surface the gate refuses is one
  error at the door and never a rejection of a desk no turn awaited. `frontOfHouse` is exported.
