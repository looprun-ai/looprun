 ...
 * so two bare `cancelMove` acts emitted in ONE step are both gated (beforeToolCall) before either
 ...
 * revert the guard change and the first `it` goes green→red), and every legal cancel flow (simulation,
 * simulation→execute, single cancel) still passes.
 */
...
describe('destructiveThrottle counts a same-STEP sibling effect', () => {
  it('THE FIX: a same-step sibling bare EXECUTE throttles the second destructive call', async () => {
  ...
  // WHAT COUNTS AS A SIMULATION DEPENDS ON WHETHER THE CALL HAS RUN. A same-step
  // sibling has NOT executed, so `tookEffect` is `undefined` for every one of them BY CONSTRUCTION — the
  // world cannot have recorded an effect that has not happened. Only the sibling's own explicit
  // `simulate: true` declares the side-effect-free ask; a bare sibling is the act it will be.
  it('a same-step sibling SIMULATION (simulate:true) does NOT throttle — a simulation changes nothing', async () => {
    const g = destructiveThrottle(['cancelMove']);
    const ctx = craftCtx({
      tool: 'cancelMove',
      args: { moveId: 'mv_2001' },
      siblingCallsThisStep: [call('cancelMove', { args: { moveId: 'mv_2001', simulate: true } })],
    });
  ...
  it('CONTROL: a same-step MULTI-SIMULATION passes — two simulations in one step are not an effect', async () => {
    // "Simulate cancelling both of my bookings" is a legal request, and the model answers it with two
    // `simulate:true` calls in ONE step. Neither has run when the second is gated, so neither can have
    // an effect on record; vetoing the second would deny the simulation for an effect nothing has had.
    const g = destructiveThrottle(['cancelMove']);
    const ctx = craftCtx({
      tool: 'cancelMove',
      args: { moveId: 'mv_2002', simulate: true },
      siblingCallsThisStep: [call('cancelMove', { args: { moveId: 'mv_2001', simulate: true } })],
    });
    expect(await g.check(ctx)).toBeNull();
  });

  it('CONTROL: a BARE sibling is the act it will be — the cap holds from the first one', async () => {
    // A tool that cannot simulate is capped without any configuration: its bare sibling declares the
    // one act this turn is allowed.
    const g = destructiveThrottle(['cancelMove']);
    const ctx = craftCtx({
      tool: 'cancelMove',
      args: { moveId: 'mv_2002' },
  ...
  it('CONTROL: the legal CROSS-STEP tail still passes — a recorded simulation, then the execute', async () => {
    const g = destructiveThrottle(['cancelMove']);
    const ctx = craftCtx({
      tool: 'cancelMove',
      args: { moveId: 'mv_2001' },
      observed: [call('cancelMove', { args: { moveId: 'mv_2001', simulate: true }, tookEffect: false })],
    });
    expect(await g.check(ctx)).toBeNull();
  });
  ...
  it('REGRESSION FLOOR: the FIRST destructive call in a step (no earlier sibling) passes', async () => {
    const g = destructiveThrottle(['cancelMove']);
    const ctx = craftCtx({ tool: 'cancelMove', args: { moveId: 'mv_2001' }, siblingCallsThisStep: [] });
    expect(await g.check(ctx)).toBeNull();
  });
  ...
  it('REGRESSION FLOOR: a cross-STEP prior EFFECT (in observed) still throttles', async () => {
    ...
    const ctx = craftCtx({
      tool: 'cancelMove',
      args: { moveId: 'mv_2002' },
      observed: [call('cancelMove', { args: { moveId: 'mv_2001' } })],
    });
    expect(await g.check(ctx)).toBeTruthy();
  });
  ...
  it('BACKEND-AGNOSTIC: with siblingCallsThisStep ABSENT (alien/one-call-per-step), the guard falls back to observed', async () => {
    ...
    const clean = craftCtx({ tool: 'cancelMove', args: { moveId: 'mv_2001' } });
...
describe('full loop — a same-step bulk destructive is throttled to ONE effect', () => {
  ...
  it('turn-1 emits TWO bare deleteItem acts in one step → the SECOND is vetoed by the throttle', async () => {
    // Turn 0 simulates BOTH records, so the engine raises a question for each. Turn 1's message carries
    // both codes, then emits both bare acts in a SINGLE scripted step (one array = one model
